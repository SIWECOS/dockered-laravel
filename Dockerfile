...
RUN apt-get update -y \
    && apt-get install -y redis-server libicu-dev openssl vim zip unzip git libpng-dev libpq-dev zlib1g-dev python3 python3-pip locales \
    && sed -i 's/# en_US.UTF-8 UTF-8/en_US.UTF-8 UTF-8/' /etc/locale.gen \
    && sed -i 's/# de_DE.UTF-8 UTF-8/de_DE.UTF-8 UTF-8/' /etc/locale.gen \
    && locale-gen \
    && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer \
    && docker-php-ext-configure intl \
    && docker-php-ext-install bcmath gd intl mbstring mysqli pdo pdo_mysql pdo_pgsql zip \
